...
public class Offlineimpl implements ioffline {
		...
		try {
			...
			if(file.exists()){
				...
				if(max.size() > 0){
					...
					sv.addlastRow(max.get(max.size()-1), "addclt");
				}
		...
		try {
			...
			if(file.exists()){
				...
				if(max.size() > 0){
					Collections.sort(max);
					StockVirtual sv = new StockVirtual(context);
					sv.addlastRow(max.get(max.size()-1), "pay");
				}
			}
	...
	public long shynchornizeTournee(List<Tournee> in) {
		...
		try {
			...
			if(file.exists()){
				...
				if(in.size() > 0){
					for (int i = 0; i < in.size(); i++) {
						pout.println("["+gson.toJson(in.get(i),Tournee.class)+"]");
						ix =1;
					}
				}
				pout.close();
			...
			// TODO: handle exception
			Log.e("save tournee",e.getMessage()  +" << ");
			ix =-1;
		}
		return ix;
	}
	...
	public List<Tournee> LoadTourneeList(String fl) {
		// TODO Auto-generated method stub
		List<Tournee> list = new ArrayList<Tournee>();

		try{
			int n;

			File file = new File(path, "/tournee.txt");
			if(file.exists()){
				//Log.e("data loaded exist  ",file.getAbsolutePath());
				File secondInputFile = new File(file.getAbsolutePath());
				InputStream secondInputStream = new BufferedInputStream(new FileInputStream(secondInputFile));
				...
				while ((line = r.readLine()) != null) {
					...
					for (int i = 0; i < jr.length(); i++) {
						JSONObject json = jr.getJSONObject(i);
						Tournee c = new Tournee();
						c = gson.fromJson(json.toString(), Tournee.class);
						list.add(c);
					}
				}
			}
			...
			Log.e("load Tournee save",e.getMessage()  +" << ");
		}
		return list;
	}
